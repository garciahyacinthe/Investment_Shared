...
from databases.paths import QuotesPath, BacktestResultsPath
...
def run_strat():
    ...
    strat_raw_result.to_csv(BacktestResultsPath + 'daily_trend_threshold_entry_raw_result.csv')
    strat_analyzed_result.to_csv(BacktestResultsPath + 'daily_trend_threshold_entry_analyzed_result.csv')
...
def enrich_data(
    ...
    closeopen_df['abs_return'] = \
        abs((closeopen_df['Open'] - closeopen_df['CloseShifted']) / closeopen_df['CloseShifted'])
...
def apply_logic(
    ...
    try:
        ...
        abs_daily_ret = close_df.loc[close_df.index == date.date()]['abs_return'][0]
    except:
...
# If BCBCLI is green then market is bullish
    if daily_ret > 0 and abs_daily_ret > 0.002:

        intra_to_use = intra_dfs[way_dict['bullish']]

    elif daily_ret < 0 and abs_daily_ret > 0.002:

        intra_to_use = intra_dfs[way_dict['bearish']]

    else:
        print('No entry for ' + str(date.date()))
        return pd.DataFrame
...
def analyze_results(res):
    ...
    # Results gathering
    macro_results['date'] = res['date'].unique()[0]
    ...
    macro_results_df.set_index('date', inplace=True)
